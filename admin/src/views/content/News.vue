<template>
  <el-card>
    <div class="toolbar">
      ...
      <el-button type="success" @click="openDialog()">发布新闻</el-button>
    </div>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑新闻' : '发布新闻'" width="620px">
...
async function load(page = query.pageNum) {
  ...
  loading.value = true
  try {
    const res = await news.list({ ...query, title: query.keyword })
...
async function onRemove(row) {
  await ElMessageBox.confirm(`确认删除新闻「${row.title}」？`, '提示', { type: 'warning' })

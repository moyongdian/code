...
微信 WXML 的表达式能力远弱于 JavaScript，很多写法会**直接编译报错**。本脚本专门拦截这类问题：

| 不支持的写法 | 示例 |
|---|---|
| 对括号表达式取下标 | `{{(user.name \|\| user.username \|\| 'U')[0]}}` ❌ |
| 对含运算符的表达式取下标 | `{{(a ? b : c)[0]}}` ❌ |
| 调用字符串方法 | `{{name.slice(0,1)}}`、`{{price.toFixed(2)}}` ❌ |
| 调用数组方法 / 箭头函数 | `{{list.filter(x => x.a)}}` ❌ |
| `Math` / `Date` / `new` / `function` | `{{Math.max(a,b)}}` ❌ |

**正确做法**：在 JS 里算好，模板只做输出。例如首字母占位：
...
```html
<!-- WXML：直接输出 -->
<view>{{item.nameInitial}}</view>
```

同时校验：页面四件套是否齐全、tabBar 路径/图标是否有效、`app.json` 声明的页面与磁盘目录是否一致、WXML 标签是否闭合、事件处理函数是否存在。
...
桩掉 `wx.*` API，**直接加载真实的页面 JS、`api`、`utils`**，打到运行中的后端。

覆盖 13 组、64 项断言：登录 → 首页 → 商家详情 → 加购/改量/收藏 → 购物车 → 地址 CRUD → 确认订单金额 → 下单事务 → 订单状态流转（待支付→待发货→已完成）→ 评价 → 收藏/我的评价 → 个人中心 → 未登录拦截 → 数据清理。

> 注意：`verify.js` 覆盖的是**数据契约与页面逻辑**，不校验 WXML 表达式语法（微信编译器才能发现），因此两者都要跑。
